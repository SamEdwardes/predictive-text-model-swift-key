...
predict_next_word <- function(input_text){
    ...
    # create three different ngrams
    search_ngrams3 <- tokens_ngrams(input_toks, 3)
    search_ngrams2 <- tokens_ngrams(input_toks, 2)
    search_ngrams1 <- tokens_ngrams(input_toks, 1)
    
    # get the last n_gram and add regular express syntax
    search_term3 <- paste0("^", tail(search_ngrams3[[1]], 1), "_")
    search_term2 <- paste0("^", tail(search_ngrams2[[1]], 1),  "_")
    search_term1 <- paste0("^", tail(search_ngrams1[[1]], 1), "_")
    
    
    # n_grams must be at least length of 2 in so search can return a result
    search.dfm <- train.dfm %>% filter (ngrams >= 2)
    
    # create search terms
    search_terms <- c(search_term1, search_term2, search_term3)
    # if(length(search_ngrams3[[1]] == 0)) rm(search)
    
    # search the dataframe for potential matches
    potential_matches <- data.frame()
    ngrams_searched = 0
    searches <- for(search_term in search_terms){
        ngrams_searched = ngrams_searched + 1
        # break out of the loop if ngram2 or ngram 3 does not exist
        if(nchar(search_term) <= 2) next
        temp <- search.dfm[grepl(pattern = search_term, x = search.dfm$feature), ]
        temp$ngrams.found <- ngrams_searched
        potential_matches <- union_all(temp, potential_matches)
        
    }
    
    potential_matches <- potential_matches %>%
        # create a prediction score, assigning more weight to matches with deeper ngrams
        mutate(score = ngrams.found) %>%
    ...
    potential_matches <- potential_matches %>%
        ...
        select(feature, prediction, ngrams.found, frequency.percent, ngrams, score, rank, frequency)
    ...
    return(list("input_text" = input_text,
                ...
                "search_ngrams" = c(search_ngrams1[[1]], search_ngrams2[[1]], search_ngrams3[[1]]),
                "search_term" = NULL,
                "potential_matches" = potential_matches
                ))

...
To assess the accuracy of the model data, we will test if the model is able to predict the correct word based on the top 3 suggestions.

```{r}
# read the test data
test.result.df <- fread("data/test.result.df.csv")
test.result.df <- test.result.df %>% filter(nchar(test.text)>0)
```
